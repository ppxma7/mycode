...
n_conditions = 2   # total number of conditions
i_conditions = np.arange(n_conditions)   # condition indices
condition_names = ["1-back", "2-back"]   # same length as i_conditions
n_blocks = 5     # number of blocks per condition
...
### set trigger names
block_start = [1,2]   # must be same length as i_conditions
rest_start = [3,4]   # must be same length as i_conditions
trig_target = 5
trig_nontarget = 6
resp = 7
...
df_allconds = pandas.concat([df_1back, df_2back], ignore_index=True)
...
df_targets_allconds = pandas.concat([df_targets_1back, df_targets_2back], ignore_index=True)
...
def nback(condition_names, condition_sequence, letter_sequence, target_sequence,
    ...
    #create a window
    window = visual.Window(fullscr=True,monitor="testMonitor", units="norm")
    window.color = "gray" 
