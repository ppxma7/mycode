function [yhat,thisEstimatedCurve] = PlotComponentCurves_lite(x, y, t, c, parameter)
try
	fontSize = 12;
	...
	for k = 1 : numGaussians
		...
		thisEstimatedCurve(k,:) = c(k) .* gaussian(t, means(k), widths(k));
		% Plot component curves.
		plot(x, thisEstimatedCurve(k,:), '-', 'LineWidth', 2);
		hold on;
		% Overall curve estimate is the sum of the component curves.
		%yhat(k,:) = yhat(k,:) + thisEstimatedCurve(k,:);
		legendStrings{k} = sprintf('Estimated Gaussian %d', k);
    end
    yhat =  sum(thisEstimatedCurve);
