...
bb.draw()
bb.export('file_name','numcount_ddur', ...
    'export_path',...
    '/Users/ppzma/The University of Nottingham/Michael_Sue - IBD - IBD/newResults/ddur/',...
    'file_type','pdf')
...
g.export('file_name','ddur_vs_age', ...
    'export_path',...
    '/Users/ppzma/The University of Nottingham/Michael_Sue - IBD - IBD/newResults/ddur/',...
    'file_type','pdf')
...
load('/Volumes/ares/data/IBD/STRUCTURAL/bias_corrected/CAT_outputs/march_2021_jg_disease_dur_wmv/wmv_poscontrast_DDUR_mrc2021.mat');

[R,P] = corrcoef(ymean_171, x);

figure('Position',[100 100 600 390])
clear g
g = gramm('x',x,'y',ymean_171);
g.stat_glm
g.set_point_options('markers', {'o'} ,'base_size',10)
g.set_text_options('Font','Helvetica','base_size',16)
g.set_names('x','Disease duration (years)','y','Left cerebral cortex (cm3)')
g.set_order_options('x',0)
g.set_color_options('map',[0 0 0])
g.draw()

g.update('color',ddur_grp2);
g.geom_point()
...
g.export('file_name','wmv_ddur', ...
    'export_path',...
    '/Users/ppzma/The University of Nottingham/Michael_Sue - IBD - IBD/newResults/ddur/',...
    'file_type','pdf')

thisregion = 'Left cerebral cortex';
thisfilename = 'WMV ddur';
T = table(subjects(:), ymean_171);
T.Properties.VariableNames = {'Subject',thisregion};
writetable(T,['/Users/ppzma/The University of Nottingham/Michael_Sue - IBD - IBD/newResults/grab_ROI_values/ddur/' thisfilename '.xlsx'],'FileType','spreadsheet','WriteRowNames',1)
...
view(-90,0)
file_path = '/Users/ppzma/The University of Nottingham/Michael_Sue - IBD - IBD/newResults/ddur/';
filename = ['ddur_surf_l2'];
...
g(1,1).set_names('x','Disease duration (years)','y','Left precentral gyrus (mm)','Color','Group')
...
g(1,2).set_names('x','Disease duration (years)','y','Left Central Opercular Cortex (mm)','Color','Group')
...
g(1,3).set_names('x','Disease duration (years)','y','Posterior Cingulate Gyrus (mm)','Color','Group')
...
g(2,1).set_names('x','Disease duration (years)','y','Posterior Cingulate Gyrus (mm)','Color','Group')
g(2,1).set_order_options('x',0)
g(2,1).set_color_options('map',[0 0 0])

g(2,2) = gramm('x',x,'y',clusterMeanT(:,5));
g(2,2).stat_glm
g(2,2).set_point_options('markers', {'o'} ,'base_size',10)
g(2,2).set_text_options('Font','Helvetica','base_size',16)
g(2,2).set_names('x','Disease duration (years)','y','Supplementary Motor Cortex (mm)','Color','Group')
g(2,2).set_order_options('x',0)
g(2,2).set_color_options('map',[0 0 0])

g(2,3) = gramm('x',x,'y',clusterMeanT(:,6));
g(2,3).stat_glm
g(2,3).set_point_options('markers', {'o'} ,'base_size',10)
g(2,3).set_text_options('Font','Helvetica','base_size',16)
g(2,3).set_names('x','Disease duration (years)','y','Anterior Cingulate Gyrus (mm)','Color','Group')
...
text(-90,13,['r=' num2str(round(R(1),4)) ])
text(-90,12,['p=' num2str(round(P(1),4)) ])
text(-45,13,['r=' num2str(round(R(2),4)) ])
text(-45,12,['p=' num2str(round(P(2),4)) ])
text(1,13,['r=' num2str(round(R(3),4)) ])
text(1,12,['p=' num2str(round(P(3),4)) ])
text(-90,2,['r=' num2str(round(R(4),4)) ])
text(-90,1,['p=' num2str(round(P(4),4)) ])
text(-45,2,['r=' num2str(round(R(5),4)) ])
text(-45,1,['p=' num2str(round(P(5),4)) ])
text(1,2,['r=' num2str(round(R(6),4)) ])
text(1,1,['p=' num2str(round(P(6),4)) ])



g.export('file_name','ct_ddur', ...
    'export_path',...
    '/Users/ppzma/The University of Nottingham/Michael_Sue - IBD - IBD/newResults/ddur/',...
    'file_type','pdf')
...
thisfilename = 'ct_ddur';
T = table(subjects(:), cl1, cl2, cl3,  cl5, cl6);
T.Properties.VariableNames = {'Subject','Left precentral gyrus','Left Central Opercular Cortex',...
    'Posterior Cingulate Gyrus','Supplementary Motor Cortex',...
