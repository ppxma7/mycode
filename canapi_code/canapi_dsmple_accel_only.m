...
for ii = 1:length(myfiles)
    markerFiles{ii} = [extractBefore(myfiles{ii},'.') '.txt'];
end
...
for iSub = 1:length(dataset)
    ...
    for ii = 1:4
        ...
        if contains(myfiles{thisSlice},'canapi12')
            % in this subject, 4,5,6 was right leg
            ch1_clv = thisFile(4,startMark:endMark);
            ch2_clv = thisFile(5,startMark:endMark);
            ch3_clv = thisFile(6,startMark:endMark);
            ch4_clv = thisFile(1,startMark:endMark);
            ch5_clv = thisFile(2,startMark:endMark);
            ch6_clv = thisFile(3,startMark:endMark);
        else
        ...
        % Here we scale things so they look between 0-1
        % Scale the active leg to its max, and then the passive to the
        % active . If you scale passive to its own max, then it will look
        % like the passive leg moved a lot more than it did.
        if maxVal > 0
            task_dsmpl = task_dsmpl ./ maxVal;
            nontask_dsmpl = nontask_dsmpl ./ maxVal;
        end
    ...
    if saveem
        ...
        for kk = 1:4
            ...
            thisRowName = [mypath extractBefore(myfiles{thisSlice},'.') '_rectify_INACTIVE_dsmpled.txt'];
            writematrix(thisRow,thisRowName,'Delimiter','\t');
    ...
    if saveem
        ...
        for jj = 1:flays
            ...
            hold on
            plot(signal)
            legend('Active','Non-active','ideal block')
...
run_labels = {'1barR', 'lowR', '1barL', 'lowL'};
channel_labels = {'Active Leg', 'Passive Leg'};
% Get sizes
...
% grid on;
save('canapi_xcorr_accel','ch1vch2_xcorr_peakXC')
% h = gcf;
