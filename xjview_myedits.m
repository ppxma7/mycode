...
for mm = clusterID
    ...
    % would be nice to write to a table the useful info
%     tmpt(mm) = b(6);
    if sum(contains(b,'aal3v1')) == 0
        peakguy(mm) = {'No aal'};
    else
        
        tmppeak = b(contains(b,'aal3v1'));
        peakguy(mm) = tmppeak(1); %get around multiples
    end
    
    test(mm,:) = [mm,numVoxels,peakcoord,peakintensity];
    
    mycell(1:6,mm) = transpose(report2(1:6));
end
...
thiscon = extractAfter(handles.imageFileName{1},'spm');
thisconname = ['spm' extractBefore(thiscon,'.')];
thisconpath = extractBefore(handles.imageFileName,'spmT');
thisconpath = char(thisconpath);

mytable = array2table(test);

bpeaktable = cell2table(transpose(peakguy));

mynewtable = [mytable bpeaktable];
mynewtable.Properties.VariableNames = {'Cluster','numVoxels','MNIx','MNIy','MNIz','PeakIntensity','AAL'};

mytable_locs = cell2table(mycell);
writetable(mynewtable,[thisconpath thisconname '_' mypad '_report.xlsx'])
writetable(mytable_locs,[thisconpath thisconname '_' mypad '_report_atlas.xlsx'])
return
