...
%    purpose: 
%
function params = mlrImportFreeSurfer_copy(varargin)
...
if ~exist('mrParamsDialog')
  fprintf('(mlrImportFreeSurfer) You must have mrTools in your path to run this\n');
  return
end
...
if sum(strcmp('defaultParams',varargin))==0
    defaultParams = 0;
elseif sum(strcmp('defaultParams',varargin))
    defaultParams = 1;
end
...
% check directories
if ~isfolder('surf'),fprintf('(mlrImportFreeSurfer) Could not find surf directory\n');return,end
if ~isfolder('mri'),fprintf('(mlrImportFreeSurfer) Could not find mri directory\n');return,end

%if ieNotDefined('baseName'), baseName = getLastDir(pwd);end

if sum(strcmp('baseName',varargin))==0
    baseName = getLastDir(pwd);
end
...
if mlrIsFile(anatFile)
  ...
  if ~isempty(mriConvert)
    fprintf('(mlrImportFreeSurfer) Converting the volume anatomy to nifti format\n')
...
for i = 1:length(hemi)
  ...
  if mlrIsFile(surfFile)
    ...
    fprintf('(mlrImportFreeSurfer) Could not find inner (white matter) surface %s\n',getLastDir(surfFile,2));
  end
  ...
  if mlrIsFile(surfFile)
    ...
    fprintf('(mlrImportFreeSurfer) Could not find outer (pial/gray matter) surface %s\n',getLastDir(surfFile,2));
  end
  ...
  if mlrIsFile(surfFile)
    ...
    fprintf('(mlrImportFreeSurfer) Could not find inflated surface %s\n',getLastDir(surfFile,2));
  end
  ...
  if mlrIsFile(curvFile)
    ...
    fprintf('(mlrImportFreeSurfer) Could not find curvature file %s\n',getLastDir(curvFile));
  end
end
...
fprintf('(mlrImportFreeSurfer) Constructing the gray graph\n')
...
return;
% fprintf('(mlrImportFreeSurfer) Fixing nifti header so that the volume is centered'))
